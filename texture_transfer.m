function [imOut] = texture_transfer(target, sample, outSize1, outSize2, patchSize, overlap, tol, alpha)
    
    %%Read input and init output
    sample = im2double(sample);
    target = im2double(target);
    inSize = size(sample);
    imOut = zeros(outSize1, outSize2, 3);
    ...
    for y=1: (patchSize - overlap): (outSize1 - patchSize + 1),
        for x=1: (patchSize - overlap): (outSize2 - patchSize +  1),
